<?php
  ...
  class Related_Sub_Image_Walker extends Walker_Nav_Menu {
    ...
    function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
      ...
      if(z_taxonomy_image_url($item_post_id)){
        $image = getThumbByUrl(z_taxonomy_image_url($item_post_id), 258, 111, "default.jpg"); 
      }
            // GET IMAGE
      $args = array(
      ...
      while($query->have_posts()) { $query->next_post(); $post_id = $query->post->ID;
        ...
        if( get_post_thumbnail_id($post_id) ){ 
          $image = getThumb($post_id, 258, 111, "default.jpg");
        }else{
          $image = "<div class=\"nav_image\">". getThumbByUrl($url, 258, 111, "default.jpg") ."</div>";
        }
      ...
      $class_names = ' class="' . esc_attr($class_names) . ' col-lg-3"';
      ...
      $item_output = 
        "<a $attributes href=\"{$permalink}\" class=\"image\">". $image .'</a>'.
        "<a $attributes href=\"{$permalink}\" class=\"title\">". $title .'</a>'. 
        $description.
        "<a $attributes href=\"{$permalink}\" class=\"more\">". _("Ver mais") .'</a>'. $args->link_after;
        
      $output .= apply_filters('walker_nav_menu_start_el', $item_output, $item, $depth, $args);
                 
      // if ( $args->description ) $output .= sprintf(' <small class="nav_desc">%s</small>', esc_html($item->description)); 
